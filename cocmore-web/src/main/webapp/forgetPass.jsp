...
<head>
	...
	<title>云筑圈管理系统V1.0-找回密码--成都云筑科技有限公司</title>
	...
	<style>
		...
		.pass-input-container  span{width:60px;float:left;}
		#account{
				color: #333;
			    font-size: 14px;
			    height: 16px;
		...
		#veritycode{
			    ...
			    vertical-align: middle;
			    width: 145px;}
	    #createCheckCode{vertical-align: middle;height:30px;}
...
<div id="wrapper" class=""> 
	...
	<div id="content">   
			<ul class="mod-sub-nav">  
				<li class="mod-sub-list list-active">1、确认帐号  </li>
			...
			<form action="javascript:validate()" method="post" id="forgotsel"> 
						<div class="mod-step-detail"> 
							...
							<div class="pass-input-container" id="pass-auth-select"> 
								<input type="text" class="pass-input" name="username" placeholder="用户名" value="" id="account" onblur = "checkAccount()"> 
								<label class="accountMsg"></label> 
								<span class="pass-input-msg"></span> 
							</div> 
							<div class="pass-input-container vcode-container clearfix"> 
								<input type="text" class="pass-input vcode-input" name="veritycode" placeholder="验证码" value="" id="veritycode"> 
						...
						<div>
							<input type="submit" name="" value="下一步" class="pass-button-submit" id="submit"/><input type="button" name="" value="返回登陆" class="pass-button-submit"/> 
						</div> 
					</div> 
			</form> 
	</div> 
...
function checkAccount(){
	...
	$.ajax({ 
		type: "POST",// 指定是post还是get,当然此处要提交,当然就要用post了   
         url: getPath()+"/user/checkUserFaccount",  //发送请求的地址。   
         data: "faccount="+account,//发送到服务器的数据   
...
function validate(){  
    ...
    $.ajax({ 
		type: "POST",// 指定是post还是get,当然此处要提交,当然就要用post了   
         url: getPath()+"/user/checkInputCode",  //发送请求的地址。   
         data: "inputCode="+inputCode,//发送到服务器的数据   
         dataType: "json",//返回纯文本字符串 timeout:20000,// 设置请求超时时间（毫秒）。   
         error: function () {//请求失败时调用函数。  
