...
keel.UploadPanel = function(cfg){
	...
	keel.UploadPanel.superclass.constructor.call(this,{				
		tbar : [
			{text:'添加文件',iconCls:'common_add',ref:'../addBtn'},'-',
			{text:'上传',ref:'../uploadBtn',iconCls:'common_top',width:80,handler:this.startUpload,scope:this},'-',
			{text:'停止上传',ref:'../stopBtn',iconCls:'common_stop',handler:this.stopUpload,scope:this,disabled:true},'-',
			{text:'删除所有',ref:'../deleteBtn',iconCls:'common_delete',handler:this.deleteAll,scope:this},'-'
		],
		layout : 'fit',
		items : [this.gp],
		listeners : {
...
//		Ext.getCmp('finfoImages').setValue(action.result.obj);
//		var me = this.customSettings.scope_handler;
//		var ds = me.gp.store;
		if (Ext.util.JSON.decode(serverData).success) {
			var src = Ext.util.JSON.decode(serverData).obj;
			if(null!=Ext.getCmp('pfinfoImages').getValue()&&""!=Ext.getCmp('pfinfoImages').getValue()){
				Ext.getCmp('pfinfoImages').setValue(Ext.getCmp('pfinfoImages').getValue()+','+src);
			}else{
				Ext.getCmp('pfinfoImages').setValue(src);
			}
			
			//Ext.get('finfoImages').up('.x-form-item').setDisplayed(false);
			cp_photoSrc(src);
//			alert(JSON.stringify(Ext.util.JSON.decode(serverData).obj));
		}
//			for(var i=0;i<ds.getCount();i++){
//				var rec =ds.getAt(i);
				...
				items : [
					{
						...
						xtype:'uploadPanel',
						border : false,
						fileSize : 1024*1024*1,//限制文件大小
						uploadUrl : getPath() + 'base/assi!doNotNeedSessionAndSecurity_uploadListpicture.ad',
						flashUrl : getPath() + 'ext/swfupload.swf',
						filePostName : 'file', //后台接收参数
						fileTypes : '*.jpg;*.png;*.gif;*.jpeg'//可上传文件类型
	//					postParams : {wpalId:wpalId} //上传文件存放目录
					}
				]
			});
		 }
//		Ext.getCmp('yunzo_swfUp').settings({file_upload_limit :""+(6-imgCount)});
		add_win.show();
	}