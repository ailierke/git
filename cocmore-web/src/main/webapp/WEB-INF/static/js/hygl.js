...
//excel模板设置
function mbsz(){
	var export_url = getPath() + '/trade/download';
...
//上级行业
var fsuperTradeStore = new Ext.data.Store({
	proxy : new Ext.data.HttpProxy({ 
		disableCaching :false,
		method:'post',
		url : getPath() + '/trade/findAllTrade'
...
var editHyglForm = new Ext.form.FormPanel({
     border : false,
	 frame	: true,
	 labelAlign: 'left',
	 buttonAlign: 'center',
	 ...
	 items:[{xtype:'hidden',id:'fid',name:'fid'},{
			...
			xtype:'combo',
	        store : fsuperTradeStore,
		 	fieldLabel:'上级行业',
	        id:'fsuperTradeId',
	        hiddenName:'fsuperTradeId',
	        triggerAction : 'all',
		    lazyInit : true,
		    mode:'local',
		    valueField : 'fid',
...
function editData(row){
	...
	if(row!='add'){
	     ...
	     if(!row){
	    	 alertWarring("请选择需要编辑的信息!");return;
	     }
	}else{
		getNumberStore();
	}
         ...
         if(!editWin){
            editWin = new Ext.Window({
			    ...
			    buttons:[{
				    text: '保存',
				    handler:function(){
				    	if(editHyglForm.form.isValid()){
				    		editHyglForm.form.submit({
				                   method:'POST',
				                   waitMsg:'正在提交.....',
								   waitTitle:'请稍等',
				                   success:function(form, action){
			    ...
			    listeners:{hide:function(){editHyglForm.form.reset();}}
            });
		}
       fsuperTradeStore.load();
        setTitleAndUrl(row, editWin);
        
	    var date = new Date();  // 得到系统日期
	    var dateString = date.getFullYear()+"-"+(date.getMonth()+1)+"-"+date.getDate()+" "+date.getHours()+":"+date.getMinutes()+":"+date.getSeconds();
	    editHyglForm.getForm().findField('fbillState').setValue(0);
	    editHyglForm.getForm().findField('fcreateTime').setValue(dateString);
	    editHyglForm.getForm().findField('fcreaterId').setValue(getUser()[1]);	
	    
	    
	    if(row != 'add') {
	    	editHyglForm.getForm().findField('fmodifiedTime').setValue(dateString);
		    editHyglForm.getForm().findField('fmodifiedId').setValue(getUser()[1]);
		    editHyglForm.getForm().findField('flastModifiedTime').setValue(dateString);
		    editHyglForm.getForm().findField('flastModifiedId').setValue(getUser()[1]);
	      	 loadData(row);
		}
	}

function loadData(row) {
	fsuperTradeStore.load({params:{},callback:function(){
			editHyglForm.form.setValues({
				fid 					: row.get('fid'),
				fnumber  		: row.get('fnumber'),
				fname    			: row.get('fname'),
				fsuperTradeId: row.get('fsuperTradeId'),
...
function setTitleAndUrl(row, win) {
	if(row == 'add') {
		win.setTitle('增加行业信息');
		editHyglForm.form.url = getPath() + '/trade/saveTrade';
	} else {
		win.setTitle('修改行业信息');
		editHyglForm.form.url = getPath() + '/trade/updateTrade';
	}
	win.show();
}
...
function gridSubAjax(param,stateId)
{
	...
	Ext.Ajax.request({
		url : getPath() + '/trade/updateTradeStatus',
		params : {fid : param , stateId : stateId},
